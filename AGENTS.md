...
**重绘触发**：输入事件本身不触发重绘；仅 Resize/DpiChange/ThemeChanged 或控件内部调用 `mark_dirty()` 时，`dispatch()` 尾部才调用 `request_frame()`。**hover 状态由 hit_test 结果唯一驱动**：EventRouter 在 MouseMove 目标变化时直接 `prev->set_hovered(false)` / `(*target)->set_hovered(true)`（`set_hovered` 内部 exchange 比较变化才 `mark_dirty()`）——任意时刻仅命中目标 hovered（堆叠/边界场景正确）；ButtonInput 不处理 hover（只响应点击）

### 6. Widget 系统 (`neko::widget`)

- **Widget** 基类：虚方法 `layout(Vec4I, Context&)`、`draw(Vec4I, Context&, backend::DirectX11&) -> Rect`、`input(Context&, Event)`、`hit_test(Mouse) -> bool`（build/event 已删除），默认实现遍历 `behaviors_` 容器按类型委托给行为组件（无对应行为时空操作/`draw` 返回 `{}`/`hit_test` 返回 false）。Public：`add_behavior<T>(Args&&...) -> T&`（make_unique 存入容器并返回引用，自动注入 `*this` 为首参）、`get_bounds()`（经 `geometry_` 观察指针读控件持有的 GeometryState，树遍历兼容）、`set_geometry()`/`set_interaction()`（控件构造绑定）、`geometry()`（非 const 访问器，父布局写子定位用）、`set_hovered()`/`get_hovered()`（经 `interaction_` 观察指针操作 InteractionState，变化时 mark_dirty）。Protected 成员：`isDirty`（`std::atomic_bool`）、`context_`（`std::weak_ptr<Context>`，所有使用点 lock，失败降级）、`behaviors_`（`std::vector<std::unique_ptr<Behavior>>`）、`geometry_`（`behavior::GeometryState*` 观察指针，生命周期 = 控件，与 `parent_` 同模式）、`interaction_`（`behavior::InteractionState*` 观察指针）。私有成员：`children_`（`MutableWidget`，变体存储 `shared_ptr<Widget>`）、`z_index_`、`id_`、`path_`
  - **共享状态**（`Behavior/`）：`InteractionState`（hovered，Input 写/Draw 读）与 `GeometryState`（bounds，布局写/绘制命中输入读）——控件持有成员，行为构造注入引用（生命周期 = 控件）
