...
- **Widget** 基类：虚方法 `layout(Vec4I, Context&)`、`draw(Vec4I, Context&, backend::DirectX11&) -> Rect`、`input(Context&, Event)`、`hit_test(Mouse) -> bool`（build/event 已删除），默认实现遍历 `behaviors_` 容器按类型委托给行为组件（无对应行为时空操作/`draw` 返回 `{}`/`hit_test` 返回 false）。Public：`add_behavior<T>(Args&&...) -> T&`（make_unique 存入容器并返回引用，自动注入 `*this` 为首参）、`get_bounds()`（经 `geometry_` 观察指针读控件持有的 GeometryState，树遍历兼容）、`set_geometry()`（控件构造绑定）、`geometry()`（非 const 访问器，父布局写子定位用）。Protected 成员：`isDirty`（`std::atomic_bool`）、`context_`（`std::weak_ptr<Context>`，所有使用点 lock，失败降级）、`behaviors_`（`std::vector<std::unique_ptr<Behavior>>`）、`geometry_`（`behavior::GeometryState*` 观察指针，生命周期 = 控件，与 `parent_` 同模式）。私有成员：`children_`（`MutableWidget`，变体存储 `shared_ptr<Widget>`）、`z_index_`、`id_`、`path_`
