...
| `Context` | 引擎共享上下文：`mark_dirty`/`widget_dirty`/`anim_inc`/`anim_dec`/`widget_tree_changed` 回调、Mouse/Keyboard 弱引用、`ColorScheme`、`StyleSheet`、`native_handle` |
...
| `Renderer` | 渲染遍历器：递归 `visit_children` + 布局偏移跟踪（`horizontal_`/`vertical_` 偏移累加），持有 `TreeManager&` 引用 |
...
| `EventRouter` | 事件分发：`std::visit` 模式匹配各事件类型 → 分发到设备更新、TreeManager::input、HitTester、主题变更、DPI 调整、调度器、销毁处理 |
...
WM_* → Platform::translate_event() → MsgPump::push_msg()
  → MsgPump::msg_loop() → EventRouter::dispatch()
    → std::visit → handle_input / handle_resize / handle_dpi_change / handle_theme_change / handle_destroy
      → TreeManager::input() / RenderScheduler::resize / Context::scheme 更新 / TreeManager::clear()
```

### 6. Widget 系统 (`neko::widget`)

- **Widget** 基类：虚方法 `draw(Vec4I, Context&, Backend&) -> Rect`、`build(Context&)`、`event(Context&)`、`input(Context&, Event)`、`hit_test(Mouse) -> bool`。Protected 成员：`bounds`（`Vec4I`）、`isFocus`（`std::atomic_bool`）、`isDirty`（`std::atomic_bool`）、`class_name_`（样式表查找键）、`horizontal_`（布局方向标志）、`context_`。私有成员：`children_`（`MutableWidget`）、`z_index_`、`id_`、`path_`
  - **Builder API**：`build<T>(Args...) -> T&`（创建子 Widget 并链式配置）、`children(fn)`（lambda 作用域批量创建子节点）、`parent()`（获取父 Widget）
  - **StyledWidget CRTP**：所有具体 Widget 继承 `Stylable<Derived>`，提供 `style(string_view name) -> Derived&` 链式调用，设置 `class_name_` 用于运行时样式表查找
- **Button**：继承 `Widget` + `Stylable<Button>`，实现 `draw`（样式表查找 → ColorScheme 回退 → 背景/边框/文字居中）、`input`（左键点击 → `on_click` 回调）、`hit_test`（`Mouse::is_inside(bounds)`）。构造函数 `Button(Context&, text="", onClick=nullptr)`，链式 setter `on_click()`/`text()`
- **Column**：继承 `Widget` + `Stylable<Column>`，垂直布局容器。实现 `draw`（样式表查找 + 背景绘制）、`build`/`event`/`input`/`hit_test`。子节点遍历由 `Renderer::render` 通过 `TreeManager` 处理
- **Row**：继承 `Widget` + `Stylable<Row>`，水平布局容器。构造时设置 `horizontal_ = true`。实现 `draw`（样式表查找 + 背景绘制）、`hit_test`。子节点遍历由 `Renderer::render` 通过 `TreeManager` 处理
- **Center**：继承 `Widget` + `Stylable<Center>`，居中布局容器。实现 `draw`（样式表查找 → ColorScheme 回退 → 背景绘制 → 计算 `(parent - child) / 2` 偏移 → 子 Widget 居中放置）、`hit_test`
...
- **StyleSheet**：全局样式表，`unordered_map<string, WidgetStyle>` 映射。`add(name, WidgetStyle) -> StyleSheet&` 注册样式、`get(name) -> const WidgetStyle*` 查找。`WidgetStyle` 包含可选 `Background`、`Border`、`text_color`、`size`、`font_size`
- **Stylable\<Derived\>**：CRTP 混入类（位于 `neko::widget` 命名空间），为派生类提供 `.style(name)` 链式调用，设置 Widget 的 `class_name_` 用于运行时样式表查找
...
| CRTP | `Stylable<Derived>` 为 Widget 提供链式 `.style()` 调用 |

## Source Tree

```
NekoUI/
├── main.cpp                          # 入口：WinMain、窗口创建、消息循环、Engine 启动、样式表定义、示例 UI
│
├── NekoUI/                           # 库核心目录
...
│   │   ├── stb_truetype.h            # 嵌入式字体光栅化（583 行，gitignored — 不跟踪）
│   │   └── DirectX11/
...
│   │
│   ├── Engine/
│   │   ├── Context.hpp               # 引擎共享上下文（回调 + Mouse/Keyboard 弱引用 + ColorScheme + StyleSheet）
│   │   ├── Engine.hpp                # 引擎主类声明
...
│   │   ├── Stylable.hpp              # Stylable CRTP 基类（链式 .style(name) 设置 class_name_）
│   │   └── StyleSheet.hpp            # 全局样式表（WidgetStyle → string 映射，add/get API）
│   │
│   └── Widget/
│       ├── Widget.hpp                # Widget 基类声明（含 Builder API：build<T>/children/parent）
│       ├── Widget.cpp                # Widget 默认实现（29 行）
│       ├── Button/
│       │   ├── Button.hpp            # Button 声明（继承 Widget + Stylable）
│       │   └── Button.cpp            # Button 实现（89 行）：draw/input/hit_test/on_click/text
│       └── Layout/
│           ├── Center.hpp            # Center 布局声明（继承 Widget + Stylable）
│           ├── Center.cpp            # Center 实现（41 行）：背景绘制 + 子 Widget 居中
│           ├── Column.hpp            # Column 垂直布局声明（继承 Widget + Stylable）
│           ├── Column.cpp            # Column 实现（48 行）：背景绘制 + 样式表查找
│           ├── Row.hpp               # Row 水平布局声明（继承 Widget + Stylable）
│           └── Row.cpp               # Row 实现（44 行）：背景绘制 + 样式表查找
```
...
- **已实现**：跨平台渲染后端抽象（D3D11 实现）、HCT Material You 色彩引擎（sRGB→XYZ→CAM16→HCT 完整管线）、平台抽象（IME TSF / 11 窗口操作 / 注册表主题检测 + 强调色）、响应式 Widget 树（含焦点导航）、Widget Builder API（`build<T>()` / `children()` / `Stylable` CRTP + 样式表）、12 种速率曲线动画引擎、线程安全事件传递、系统主题变化检测与传递（Light/Dark + AccentColor）、全部核心 Widget（Button/Center/Column/Row）已实现绘制和交互
