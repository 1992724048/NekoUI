...
- **命名空间**: `neko`，子命名空间包括 `neko::type`、`neko::engine`、`neko::widget`、`neko::backend`、`neko::platform`、`neko::device`、`neko::component`、`neko::style`

## Architecture

分层架构，从底层到上层依次为：

1. **平台抽象层** (`neko::platform`) — 平台事件封装与转换。`Platform` 基类（单例）负责将原生消息（如 Win32 WM\_\*）转换为统一 `Event` 变体。支持的事件包括：鼠标/键盘输入、窗口尺寸变化、DPI 变化、窗口销毁（`DestroyEvent`，`WM_DESTROY`）、系统主题切换（`ThemeChangedEvent`，`WM_SETTINGCHANGE/ImmersiveColorSet` + 注册表/DWM 查询）。主题数据以 `style::ColorScheme` 形式存储在 `Context::scheme` 中，Widget 可直接读取各色彩角色。
2. **渲染后端** (`neko::backend`) — `Backend` 抽象基类定义绘制接口（矩形、圆形、线条、文本），`DirectX11` 为其具体实现。使用策略模式，支持切换不同渲染 API。
3. **输入设备** (`neko::device`) — 键盘/鼠标状态管理，含 DPI 感知点击测试。
...
6. **Widget 系统** (`neko::widget`) — `Widget` 基类（约束、绘制、布局、事件、子节点、命中测试），具体 Widget：`Button`（文本、颜色、点击回调、颜色动画），`Center` 布局（空存根）。
...
│       ├── Type.hpp              # 核心类型：Vec2/3/4<T>、Color
│       ├── Backend/              # 渲染后端抽象层
│       │   ├── Backend.hpp       # 抽象基类
...
- **草稿状态** — 核心架构已建立，功能尚不完整
- 已实现：跨平台渲染后端抽象、平台抽象、响应式 Widget 树、速率曲线动画引擎、线程安全事件传递、窗口销毁事件处理、系统主题变化检测与传递
- 未实现：`Layout/Center.hpp` 为空、`Style/Button/` 目录为空、仅实现 `Widget` 基类和 `Button` 两个 Widget
- 无可运行的测试
