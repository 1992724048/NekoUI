...
| `Context` | 引擎共享上下文：`mark_dirty`/`widget_dirty`/`anim_inc`/`anim_dec`/`widget_tree_changed` 回调、Mouse/Keyboard 弱引用、`ColorScheme`、`native_handle` |
...
- **Widget** 基类：虚方法 `draw(Vec4I, Context&, Backend&) -> Rect`、`build(Context&)`、`event(Context&)`、`input(Context&, Event)`、`hit_test(Mouse) -> bool`。Protected 成员：`bounds`（`Vec4I`）、`isFocus`（`std::atomic_bool`）、`isDirty`（`std::atomic_bool`）、`horizontal_`（布局方向标志）、`context_`。私有成员：`children_`（`MutableWidget`）、`z_index_`、`id_`、`path_`
...
- **Button**：继承 `Widget` + `BackgroundStyle`、`SizeStyle`、`BorderStyle`、`TextStyle`，实现 `draw`（ColorScheme 回退 → 背景/边框/文字居中）、`input`（左键点击 → `on_click` 回调）、`hit_test`（`Mouse::is_inside(bounds)`）。构造函数 `Button(Context&, text="", onClick=nullptr)`，链式 setter `on_click()`/`text()`
- **Column**：继承 `Widget` + `BackgroundStyle`、`SizeStyle`，垂直布局容器。实现 `draw`（背景绘制）、`build`/`event`/`input`/`hit_test`。子节点遍历由 `Renderer::render` 通过 `TreeManager` 处理
- **Row**：继承 `Widget` + `BackgroundStyle`、`SizeStyle`，水平布局容器。构造时设置 `horizontal_ = true`。实现 `draw`（背景绘制）、`hit_test`。子节点遍历由 `Renderer::render` 通过 `TreeManager` 处理
- **Center**：继承 `Widget` + `BackgroundStyle`，居中布局容器。实现 `draw`（ColorScheme 回退 → 背景绘制 → 计算 `(parent - child) / 2` 偏移 → 子 Widget 居中放置）、`hit_test`
...
```
NekoUI/
├── main.cpp                          # 入口：WinMain、窗口创建、消息循环、Engine 启动、示例 UI
│
├── NekoUI/                           # 库核心目录
