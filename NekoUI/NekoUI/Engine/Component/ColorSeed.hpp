...
struct ViewingConditions {
    ...
    static auto make() -> ViewingConditions {
        ...
        double adaptingLuminance = (200.0 / 3.141592653589793)
                                 * yFromLstar(50.0) / 100.0;

        double backgroundLstar = 50.0;
        double surround = 2.0; // average surround

        // Transform white point XYZ to CAT02 cone responses
        double rW = whitePoint[0] * 0.401288 + whitePoint[1] * 0.650173
                  + whitePoint[2] * -0.051461;
        double gW = whitePoint[0] * -0.250268 + whitePoint[1] * 1.204414
                  + whitePoint[2] * 0.045854;
        double bW = whitePoint[0] * -0.002079 + whitePoint[1] * 0.048952
                  + whitePoint[2] * 0.953127;
        ...
        double d = f * (1.0 - (1.0 / 3.6)
                       * std::exp((-adaptingLuminance - 42.0) / 92.0));
        ...
        double fl = (k4 * 5.0 * adaptingLuminance)
                  + 0.1 * k4F * k4F
                  * std::pow(5.0 * adaptingLuminance, 1.0 / 3.0);
        ...
        double rgbAFactors[3] = {
            std::pow(fl * rgbD[0] * rW / 100.0, 0.42),
...
struct HctSolver {
    ...
    inline static const double SCALED_DISCOUNT_FROM_LINRGB[3][3] = {
    ...
    inline static const double LINRGB_FROM_SCALED_DISCOUNT[3][3] = {
    ...
    inline static const double Y_FROM_LINRGB[3] = {0.2126, 0.7152, 0.0722};
    ...
    inline static const double CRITICAL_PLANES[255] = {
...
private:
    ...
    static auto lerpPoint(const std::array<double, 3>& source, double t,
                          const std::array<double, 3>& target) -> std::array<double, 3> {
        return std::array<double, 3>{
    ...
    static auto setCoordinate(const std::array<double, 3>& source, double coordinate,
                              const std::array<double, 3>& target, int axis) -> std::array<double, 3> {
    ...
    static auto bisectToSegment(double y, double targetHue,
                                std::array<double, 3>& outLeft,
                                std::array<double, 3>& outRight) -> void {
    ...
    static auto midpoint(const std::array<double, 3>& a,
    ...
    static auto bisectToLimit(double y, double targetHue) -> std::array<double, 3> {
        ...
        for (int axis = 0; axis < 3; axis++) {
            if (left[axis] != right[axis]) {
                ...
                for (int i = 0; i < 8; i++) {
                    if (std::abs(rPlane - lPlane) <= 1) break;
