...
namespace neko::color {
    ...
    auto ColorScheme::from_seed(const type::Color seed, const bool is_dark, const float contrast_level) -> ColorScheme {
        const auto argb = detail::argb_from_rgb(static_cast<int>(std::round(seed.r * 255.0F)), static_cast<int>(std::round(seed.g * 255.0F)), static_cast<int>(std::round(seed.b * 255.0F)));
